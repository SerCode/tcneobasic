...
//       print $q;
   ...
   public function findByName($username = '', $archive = FALSE)
   {
      ...
      $query = $this->db->prepare('SELECT * FROM ' . $table . ' WHERE username = :val1 LIMIT 0,20');
      $query->bindParam('val1', $username);
      $result = $query->execute();
   ...
   public function getAll($order1 = 'lastname', $order2 = 'firstname', $sort = 'ASC', $archive = false, $includeAdmin = false)
   {
      ...
      if ($includeAdmin)
         $query = $this->db->prepare('SELECT * FROM ' . $table . ' ORDER BY ' . $order1 . ' ' . $sort . ', ' . $order2 . ' ' . $sort . ' LIMIT 0,20');
         else
            $query = $this->db->prepare('SELECT * FROM ' . $table . ' WHERE username != "admin" ORDER BY ' . $order1 . ' ' . $sort . ', ' . $order2 . ' ' . $sort . ' LIMIT 0,20');
   ...
   public function getAllButHidden($order1 = 'lastname', $order2 = 'firstname', $sort = 'ASC', $archive = false, $includeAdmin = false)
   {
      ...
      if ($includeAdmin)
         $query = $this->db->prepare('SELECT * FROM ' . $table . ' WHERE hidden != 1 ORDER BY ' . $order1 . ' ' . $sort . ', ' . $order2 . ' ' . $sort . ' LIMIT 0,20');
         else
            $query = $this->db->prepare('SELECT * FROM ' . $table . ' WHERE username != "admin" AND hidden != 1 ORDER BY ' . $order1 . ' ' . $sort . ', ' . $order2 . ' ' . $sort . ' LIMIT 0,20');
   ...
   public function getAllLike($like, $archive = FALSE)
   {
      ...
      $query = $this->db->prepare('SELECT * FROM ' . $table . ' WHERE firstname LIKE :val1 OR lastname LIKE :val1 OR username LIKE :val1 ORDER BY lastname ASC, firstname ASC LIMIT 0,20');
      $val1 = '%' . $like . '%';
